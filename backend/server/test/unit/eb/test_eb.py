...
from backend.server.test import PROJECT_ROOT, FIXTURES_ROOT
...
class Elastic_Beanstalk_Test(unittest.TestCase):
    def test_run(self):
        ...
        tempdir = tempfile.TemporaryDirectory(dir=f"{PROJECT_ROOT}/backend/server")
        ...
        subprocess.check_call(f"git ls-files . | cpio -pdm {tempdirname}", cwd=f"{PROJECT_ROOT}/backend/server/eb", shell=True)
        import pdb
        pdb.set_trace()
    ...
    def test_config(self):
        check_config_script = os.path.join(PROJECT_ROOT, "backend", "server", "eb", "check_config.py")
