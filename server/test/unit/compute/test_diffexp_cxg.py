...
class DiffExpTest(unittest.TestCase):
    ...
    def sparse_diffexp(self, apply_col_shift):
        with tempfile.TemporaryDirectory() as dirname:
            ...
            diffexp_results_sparse = diffexp_cxg.diffexp_ttest(adaptor_sparse, self.get_mask(adaptor_sparse, 1, 10), self.get_mask(adaptor_sparse, 2, 10), 10)
            diffexp_results_dense = diffexp_cxg.diffexp_ttest(adaptor_dense, self.get_mask(adaptor_dense, 1, 10), self.get_mask(adaptor_dense, 2, 10), 10)
            ...
            for row in range(cols_sparse.shape[0]):
                for col in range(cols_sparse.shape[1]):
