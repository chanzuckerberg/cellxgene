...
class TermsPrompt extends React.PureComponent<{}, State> {
    ...
    renderTos() {
        ...
        <a style={{
            ...
            }} href={tosURL} target="_blank" rel="noopener">
          terms of service
        </a>
        .{" "}
      </span>);
    ...
    renderPrivacy() {
        ...
        <a style={{
                fontWeight: 700,
                textDecoration: "underline",
            }} href={privacyURL} target="_blank" rel="noopener">
          privacy policy
        </a>
        .&nbsp;
      </span>);
    ...
    render() {
        ...
        return (<Drawer onclose={this.drawerClose} isOpen={isOpen} size="120px" position={Position.BOTTOM} canOutsideClickClose={false} hasBackdrop /* if the user can't use the app or click outside to dismiss, that should be visually represented with a backdrop */ enforceFocus={false} autoFocus={false} portal={false}>
        <div className={Classes.DRAWER_BODY} style={{ backgroundColor: Colors.LIGHT_GRAY1 }}>
          ...
          <div className={Classes.DIALOG_FOOTER} style={{ textAlign: "left" }}>
            <Button intent="primary" onClick={this.handleOK} data-testid="tos-cookies-accept">
              I&apos;m OK with cookies!
            </Button>{" "}
...
}(window as any).cookieDecisionCallback instanceof Function) {
      try {
        (window as any).cookieDecisionCallback();
      } catch (e) {
        // continue
      }
    }
  ...
  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'handleNo'.
  handleNo = () => {
  ...
  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'renderTos'.
  renderTos() {
    // @ts-expect-error ts-migrate(2532) FIXME: Object is possibly 'undefined'.
    const { tosURL } = this.props;
  ...
  // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'renderPrivacy'.
  renderPrivacy() {
    // @ts-expect-error ts-migrate(2532) FIXME: Object is possibly 'undefined'.
    const { privacyURL } = this.props;
